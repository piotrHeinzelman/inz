...
int main(int argc, char** argv) {
	...
	while (epochs < 1000) {
		...
		int cols=28; //rgb kolumny
                int rows=28; //rzędy
		// *** FAKE LOAD IMAGE *** ///
		//INPUT copy image to memory
		for (int i = 0; i != cols; i++) {
			for (int j = 0; j != rows; j++) {
				darray[i][j] = (i+j)/5; //if you encounter an error during transfer of image data its probably here
			}
		}
