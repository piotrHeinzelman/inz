...
int main() {
   ...
   // cublasSetMatrixAsync(LEN, Lay0In, sizeof(double), Xhost, Lay0In, X0, LEN,  stream);
    ...
    checkCUDA(cudaMalloc(reinterpret_cast<void **>(&W1), sizeof(double)*Lay1In*Lay1Out ));
...
*/
    ...
    for (int i=0;i<(LEN*Lay0Out);i++){if ( Yhost[i]>.1){ std::cout<<"#";} else {std::cout<<".";} ; if ( i%(64)==0) std::cout<<std::endl;}
