...
public class _SoftMax {
    ...
    public _SoftMax(int input, int output, int filterNum) {
        this.filterNum=filterNum;
        weights = _Mat.m_scale( _Mat.m_random(input, output), 1.0f / input );
        bias = _Mat.v_zeros(10);
        String s="aaa";
    ...
    public float[][] forward(float[][][] input) {
        ...
        float[][] in = _Mat.m_flatten(input);  //1X1342
     ...
     // [1] X [10] =  [1] X [1342]  * [1342] X [10] + [1] X [10]
        output = _Mat.mm_add(_Mat.mm_mult(in, weights), bias);
        //compute softmax probabilities.
        float[][] totals = _Mat.v_exp(output);
        float inv_activation_sum = 1 / _Mat.v_sum(totals);
        //cache input
        this.input = in;
        return _Mat.v_scale(totals, inv_activation_sum);
    }
    ...
    public float[][][] backprop(float[][] d_L_d_out, float learning_rate) {
        ...
        float[][] t_exp = _Mat.v_exp(output);
        float S = _Mat.v_sum(t_exp);
        ...
        for (int i = 0; i < d_L_d_out[0].length; i++) {
            ...
            float[][] d_out_d_t = _Mat.v_scale(t_exp, -t_exp[0][i] / (S * S));
            d_out_d_t[0][i] = t_exp[0][i] * (S - t_exp[0][i]) / (S * S);
            
            d_L_d_t = _Mat.m_scale(d_out_d_t, grad);
            //gradient of totals w.r.t weights -- [1342] X [1]
            float[][] d_t_d_weight = _Mat.m_transpose(input);
            ...
            float[][] d_L_d_w = _Mat.mm_mult(d_t_d_weight, d_L_d_t);
            //gradient of Loss w.r.t. inputs ---> chain rule
            // [1342] X [1]      [1342] X [10]    *   [10] X [1](transposed)
            d_L_d_inputs = _Mat.mm_mult(d_t_d_inputs, _Mat.m_transpose(d_L_d_t));
            //gradient of loss w.r.t. bias
            float[][] d_L_d_b = d_L_d_t;
            //update the weight and bias matrices.
            weights = _Mat.mm_add(_Mat.m_scale(d_L_d_w, -learning_rate), weights);
            bias = _Mat.mm_add(_Mat.m_scale(d_L_d_b, -learning_rate), bias);
        }
        // reshape the final gradient matrix to the input shape of the maxpooling layer.
        // [1] X [1342](transposed) ----> [8] X [13] X [13]
        return _Mat.reshape(_Mat.m_transpose(d_L_d_inputs),filterNum,13,13);
    }
}
