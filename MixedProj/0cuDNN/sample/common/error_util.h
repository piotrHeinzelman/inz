...
showDevices(void) {
    ...
    for (int i = 0; i < totalDevices; i++) {
        ...
        int clockRate;
        checkCudaErrors(cudaDeviceGetAttribute(&clockRate, cudaDevAttrClockRate, i));
        int memoryClockRate;
        checkCudaErrors(cudaDeviceGetAttribute(&memoryClockRate, cudaDevAttrMemoryClockRate, i));
        printf(
